...
export async function exportPptx({ output, isPro, excludedSlides, slideOrder, deckTheme }: ExportOptions) {
  ...
  const { BG, FG, PRIMARY, MUTED, MUTED_TEXT, ACCENT } = colors;

  // ── Title slide ──
  const titleSlide = pptx.addSlide();
  titleSlide.background = { color: BG };
  titleSlide.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: "100%", h: 0.04, fill: { color: ACCENT } });
  titleSlide.addText(title, { x: 0.8, y: 1.8, w: 8.4, h: 1.2, fontSize: 36, fontFace: "Arial", bold: true, color: PRIMARY, align: "center", valign: "middle" });
  titleSlide.addText(output.mode.replace(/_/g, " ").toUpperCase(), { x: 0.8, y: 3.2, w: 8.4, fontSize: 14, fontFace: "Arial", color: PRIMARY, align: "center", charSpacing: 4 });
  ...
  for (const originalIdx of activeIndices) {
    ...
    if (categoryLabel) {
      slide.addText(categoryLabel.toUpperCase(), {
        ...
        fontSize: 10, fontFace: "Arial", bold: true, color: PRIMARY, align: "left", valign: "top",
        charSpacing: 3,
      });
    }
    ...
    slide.addText(headline, {
      ...
      fontSize: hSize, fontFace: "Arial", bold: true, color: PRIMARY, align: "left", valign: "top",
    });
