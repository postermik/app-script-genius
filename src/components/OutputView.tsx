...
export function OutputView() {
  ...
  const handleUpdateDeliverable = (updated: any) => {
    ...
    // Debounced persist to Supabase (saves 500ms after last edit)
    if (currentProjectId && updated.deckFramework) {
      if (slideSaveTimer.current) clearTimeout(slideSaveTimer.current);
      slideSaveTimer.current = setTimeout(() => {
        supabase.from("projects").select("output_data").eq("id", currentProjectId).single().then(({ data }) => {
          ...
          const updatedOutputData = {
            ...existing,
            slide_framework: { ...existing.slide_framework, deckFramework: updated.deckFramework },
          };
          supabase.from("projects").update({ output_data: updatedOutputData as any }).eq("id", currentProjectId).then(() => {
            console.log("[Persistence] Slide edits saved");
          });
        });
      }, 500);
    }
